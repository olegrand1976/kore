...
3. **Édition** : modale d'édition d'une application existante → si non liée, sélection d'un projet Taiga → `PUT /applications/{id}` avec `taigaProjectId` ; si déjà liée, affichage lecture seule (`GET /integrations/taiga/links/by-application/{id}`). La liaison Taiga est tentée **avant** la mise à jour org : un échec de liaison ne modifie pas l'application.
