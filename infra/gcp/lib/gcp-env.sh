# Defaults GCP — Kore (premedica-prod-2025)
...
AR_REPO="${AR_REPO:-kore}"
SERVICE_ACCOUNT="${SERVICE_ACCOUNT:-kore-run}"
API_SERVICE="${API_SERVICE:-kore-api}"
FRONTEND_SERVICE="${FRONTEND_SERVICE:-kore-frontend}"
DB_NAME="${DB_NAME:-kore}"
DB_USER="${DB_USER:-kore_app}"
MIGRATE_USER="${MIGRATE_USER:-kore_migrate}"
REDIS_DB="${REDIS_DB:-13}"
REDIS_KEY_PREFIX="${REDIS_KEY_PREFIX:-kore}"

REDIS_VM_NAME="${REDIS_VM_NAME:-shared-redis}"
REDIS_VM_ZONE="${REDIS_VM_ZONE:-${GCP_RUN_REGION}-b}"

CUSTOM_DOMAIN="${CUSTOM_DOMAIN:-kore.ll-it-sc.be}"
...
GITHUB_DEPLOY_SA="${GITHUB_DEPLOY_SA:-github-kore-deploy}"
