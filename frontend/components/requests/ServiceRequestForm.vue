...
const { t } = useI18n()
const { get, pickAppMethodologyProfile, pickAppEquipeIds } = useApplications()
    ...
    if (props.showAssignee) {
      ...
      applicationUsers.value = users
        .filter((user) => {
          ...
          return userEquipeIds.some((equipeId) => applicationEquipeIds.has(equipeId))
        })
        .map((user) => ({
          id: pickUserId(user),
...
const onSubmit = () => {
  ...
  emit('submit', {
    ...
    assigneeId: props.showAssignee ? (assigneeId.value || undefined) : undefined,
...
<template>
  <form class="service-request-form" @submit.prevent="onSubmit">
    <AppApplicationSelect
      ...
      required
    />
    <div v-if="showAssignee" class="service-request-form__field">
